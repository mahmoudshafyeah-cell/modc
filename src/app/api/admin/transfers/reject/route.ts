// src/app/api/admin/transfers/reject/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    if (wallet) {
      await supabase
        .from('wallets')
        .update({ reserved_balance: (wallet.reserved_balance || 0) - transfer.amount })
        .eq('user_id', transfer.user_id);
    }
    ...
    await supabase
      ...
      .update({
        status: 'rejected',
        metadata: { ...transfer.metadata, rejected_by: decoded.role, reason },
      })
      .eq('id', id);
    ...
    await supabase.from('notifications').insert({
      ...
      title: 'تم رفض تحويلك',
      message: `تم رفض تحويلك بقيمة $${transfer.amount.toFixed(2)}. السبب: ${reason || 'غير محدد'}.`,
      type: 'warning',
      read: false,
    });
    ...
    if (admins && admins.length > 0) {
      const adminNotifications = admins.map(admin => ({
        ...
        title: 'تم رفض تحويل',
        message: `تم رفض تحويل بقيمة $${transfer.amount.toFixed(2)} من ${senderProfile?.full_name || senderProfile?.email || 'غير معروف'}.`,
        type: 'admin',
        read: false,
