...
import { Bell, X, CheckCheck, ExternalLink, Wallet } from 'lucide-react';
...
export default function DashboardTopbar({ userData }: DashboardTopbarProps) {
  ...
  return (
    <header className="h-16 flex items-center justify-between px-6 border-b flex-shrink-0"
      ...
      <div className="flex items-center gap-4">
        {/* يمكن إضافة رابط أو شعار */}
      </div>

      <div className="flex items-center gap-3">
        {/* ✅ عرض الرصيد الحقيقي للمحفظة */}
        {balance !== null && (
          ...
          {notifOpen && (
            <div className="absolute top-full left-0 mt-2 w-80 max-h-96 overflow-y-auto rounded-2xl z-50 animate-slide-up"
                ...
                {loadingNotifs ? (
                  ...
                  notifications.map(n => (
                    <div key={n.id}
                      ...
                      className={`p-4 text-right transition-colors hover:bg-violet-500/10 cursor-pointer ${!n.read ? 'bg-violet-500/5' : ''}`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            {!n.read && <div className="w-2 h-2 rounded-full" style={{ background: n.color || '#00D4FF' }} />}
              ...
              <div className="p-3 text-center border-t" style={{ borderColor: `${accentColor}10` }}>
                <button onClick={markAllAsRead} className="flex items-center gap-1.5 mx-auto text-xs text-violet-400 hover:text-violet-300 transition-colors font-semibold">
                  <CheckCheck size={13} />
                  تحديد الكل كمقروء
                </button>
              </div>
        ...
        <Link href="/customer-dashboard"
          className="flex items-center gap-2 px-3 h-9 rounded-xl transition-all duration-200 hover:bg-violet-500/10"
