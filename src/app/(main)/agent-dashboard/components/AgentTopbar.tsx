...
interface NavItem {
  ...
  tab?: string; action?: string; badge?: string; disabled?: boolean;
}
...
export default function AgentSidebar({
  ...
  const [footerCopyright, setFooterCopyright] = useState('© 2025 ModC - وكيل');
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        ...
        <div className="px-4 pt-6 pb-4">
          <div className="rounded-2xl p-4" style={{ background: 'rgba(12, 113, 178, 0.1)', backdropFilter: 'blur(12px)', border: '1px solid rgba(12, 113, 178, 0.2)' }}>
            ...
            <div className="text-center mb-3">
              <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold" style={{ background: 'rgba(12, 113, 178, 0.15)', color: '#0c71b2' }}>{isSubAgent ? 'وكيل فرعي' : 'وكيل معتمد'}</span>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 text-xs text-gray-400"><CreditCard size={14} /><span>{isSubAgent ? 'محفظة وكيل فرعي' : 'محفظة وكيل'}</span></div>
          </div>
        </div>
        ...
        <nav className="py-2 px-3">
          <ul className="space-y-0.5">
            {agentNavItems.map(item => {
              ...
              return (
                <li key={item.id}>
                  <button onClick={() => handleNavClick(item)} disabled={disabled}
        ...
        <div className="px-3 py-2">
          <ul className="space-y-0.5">
            {bottomNavItems.map(item => {
              ...
              return (
                <li key={item.id}>
                  <Link href={item.tab === 'about' ? '/legal/about' : item.tab === 'terms' ? '/legal/terms-of-service' : '/legal/privacy-policy'} onClick={() => setMobileOpen(false)}
